...
class MapApp {
  ...
  populateInterviewerDropdown() {
    ...
    interviewerSelect.addEventListener('change', (e) => {
      ...
      if (this.selectedInterviewer === 'all') {
        ...
        this.datesWithMarkers = this.generateDatesWithMarkers(this.markers.filter(marker => marker.interviewer === this.selectedInterviewer));
        this.resetAnimation();
  ...
  handleDOMEvents() {
    document.addEventListener('DOMContentLoaded', () => {
      document.getElementById('play-pause').addEventListener('click', this.togglePlayPauseAnimation.bind(this));
      ...
      document.getElementById('reset').addEventListener('click', this.resetAnimation.bind(this));
    });
  }
  ...
  getMarkersForDate(date) {
    if (this.selectedInterviewer === 'all') {
      ...
      return this.markers.filter(marker => 
        marker.interviewer === this.selectedInterviewer && marker.date === date
      );
    }
  }
  ...
  displayMarkersForDate(date) {
    ...
    if (this.isAnimationPlaying) {
      ...
      if (this.currentDateIndex < this.datesIn1942.length) {
        this.animationInterval = setTimeout(() => this.displayMarkersForDate(this.datesIn1942[this.currentDateIndex]), timeoutDuration);
      } else {
        this.pauseAnimation();
        this.currentDateIndex = 0;
      }
  ...
  displayMarkersForInterviewer(interviewerName) {
    this.markers.forEach(marker => marker.setMap(null));
    this.markers.filter(marker => marker.interviewer === interviewerName)
...
// Instantiate and expose MapApp globally if necessary.
